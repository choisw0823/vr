    ...
    def build_audio_encoder(self):
        ...
        if self.config.model.audio_encoder.name == 'beats':
            ...
            if 'cfg' in checkpoint:
                ...
                # Use default config if cfg is not available
                audio_cfg = BEATsConfig()
            
            # Fix negative input_patch_size issue
            if hasattr(audio_cfg, 'input_patch_size') and audio_cfg.input_patch_size <= 0:
                audio_cfg.input_patch_size = 16  # Set reasonable default
            BEATs_model = BEATs(audio_cfg)
            if 'model' in checkpoint:
                msg = BEATs_model.load_state_dict(checkpoint['model'], strict=False)
            else:
                msg = BEATs_model.load_state_dict(checkpoint, strict=False)
            logger.info(msg)
            return BEATs_model
