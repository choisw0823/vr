...
from ..utils.dataset import RLHFDataset, RankLLMRerankDataset, collate_fn
...
def create_dataloader(config: DataConfig,
    ...
    # 공통 kwargs (train/val 공용)
    # - format_prompt: easy-r1 Jinja 템플릿 경로로 사용
    # - frames_per_candidate: 지정 없으면 RankLLMRerankDataset 내부 기본/metadata 사용
    common_kwargs = dict(
        tokenizer=tokenizer,
        processor=processor,
        jinja_prompt_path=config.format_prompt,            # ✅ Jinja 템플릿(.jinja) 파일 경로
        image_root=config.image_dir,                       # 후보 프레임/비디오가 상대경로일 때 prefix
        frames_per_candidate=getattr(config, "frames_per_candidate", None),  # 선택
        max_prompt_length=config.max_prompt_length,
        truncation="right",
        min_pixels=config.min_pixels,
        max_pixels=config.max_pixels,
        video_fps=config.video_fps,
        allow_video_decode=getattr(config, "allow_video_decode", False),
        filter_overlong_prompts=config.filter_overlong_prompts,
        filter_workers=getattr(config, "filter_overlong_prompts_workers", 16),
    )

    # ---- Train dataset / dataloader ----
    train_dataset = RankLLMRerankDataset(
        data_path=config.train_files,                      # JSON/dir/HF hub 모두 지원
        **common_kwargs,
    )

    # 체크포인트 재개를 위한 sampler 구성 동일
    if config.shuffle:
        g = torch.Generator()
    ...
    val_dataset = RankLLMRerankDataset(
        data_path=config.val_files,
        **common_kwargs,
    )
