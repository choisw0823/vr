#!/usr/bin/env python3
"""
MSRVTT Multimodal Similarity Matrix Extraction Script
Extract all text-video similarities using multimodal encoder (cross-attention)
Train 9k: random 1 caption per video -> 9,000 texts
Test 1k: 1,000 texts
Total: 10,000 texts × 10,000 videos = 100M similarity pairs
"""
...
def load_msrvtt_data():
    """Load raw MSRVTT data (videos and texts)"""
    logger.info("Loading MSRVTT annotation files...")
    
    # Load test and train annotations
    test_file = "/home/work/smoretalk/seo/reranking/new_d/MSR-VTT/msrvtt_test_1k.json"
    train_file = "/home/work/smoretalk/seo/reranking/new_d/MSR-VTT/msrvtt_train_9k.json"
    ...
    # Load test data (1 caption per video)
    with open(test_file, 'r') as f:
        test_data = json.load(f)
    
    for video_info in test_data:  # Direct list, not test_data['videos']
        video_id = video_info['video_id']
        video_path = os.path.join(video_root, f"{video_id}.mp4")
        
        if os.path.exists(video_path):
            # Caption is a string in test data
    ...
    # Load train data (20 captions per video, sample 1)
    with open(train_file, 'r') as f:
        train_data = json.load(f)
    
    for video_info in train_data:  # Direct list, not train_data['videos']
        video_id = video_info['video_id']
        video_path = os.path.join(video_root, f"{video_id}.mp4")
        
        if os.path.exists(video_path):
            # Caption is a list in train data
    ...
    logger.info(f"Loaded {len(all_videos)} videos and {len(all_texts)} texts")
...
def main(config):
    ...
    # Load raw MSRVTT data
    logger.info("Loading raw MSRVTT data...")
    all_videos, all_texts, video_text_mapping = load_msrvtt_data()
    
    # Setup cache directory (shared across all runs)
    cache_dir = "./outputs/msrvtt_features/token_cache"
    ...
    # Compute multimodal similarity matrix using cross-attention
    logger.info("Computing multimodal similarity matrix with cross-attention...")
    output_dir = os.path.join(config.output_dir, "multimodal_similarities")
    ...
    with open(meta_path, 'w') as f:
        json.dump({
            ...
            'description': 'Multimodal similarity matrix using cross-attention from raw videos and texts'
        }, f, indent=2)
    
    logger.info(f"Metadata saved to {meta_path}")
    logger.info("✅ Multimodal similarity extraction completed!")
