...
class SFTDataGenerator:
    def __init__(self, 
                 ...
                 video_base_path: str = "/home/work/smoretalk/seo/reranking/new_d/MSR-VTT/video",
                 output_dir: str = "./sft_data_contrastive_reasoning_unique_v1",
    ...
    def select_samples(self, data: Dict[str, Any], num_samples: int = 16, processed_indices: set = None) -> List[Dict]:
        """샘플 선택 (기존에 처리되지 않은 것들만)"""
        if processed_indices is None:
            processed_indices = set()
        
        print(f"{num_samples}개 샘플을 선택하는 중... (기존 처리된 {len(processed_indices)}개 제외)")
        
        # 다양한 유사도 범위에서 샘플 선택
        samples = data['training_samples']
        
        # 유사도 범위별로 샘플 분류 (기존 처리된 것 제외)
        high_similarity = []
        medium_similarity = []
        low_similarity = []
        ...
        print(f"사용 가능한 샘플: 고유사도 {len(high_similarity)}개, 중유사도 {len(medium_similarity)}개, 저유사도 {len(low_similarity)}개")
        ...
        for i, range_samples in enumerate(ranges):
            ...
            if len(range_samples) >= samples_per_range and samples_per_range > 0:
        ...
        while len(selected) < num_samples:
            remaining = [s for s in samples if s not in selected and samples.index(s) not in processed_indices]
            if remaining:
    ...
    def process_sample(self, sample: Dict, sample_idx: int) -> Dict[str, Any]:
        ...
        for i, video in enumerate(videos):
            ...
            if frames:
                ...
                # 프레임 저장
                frame_dir = f"{self.output_dir}/frames/sample_{sample_idx}_video_{i+1}"
    ...
    def load_existing_data(self) -> Dict[str, Any]:
        ...
        if os.path.exists(all_file):
            ...
            with open(all_file, 'r', encoding='utf-8') as f:
                ...
                existing_data["all_samples"] = all_data.get("samples", [])
                print(f"기존 전체 샘플: {len(existing_data['all_samples'])}개")
        
        if os.path.exists(kept_file):
            with open(kept_file, 'r', encoding='utf-8') as f:
                kept_data = json.load(f)
                existing_data["kept_samples"] = kept_data.get("samples", [])
                print(f"기존 채택된 샘플: {len(existing_data['kept_samples'])}개")
        
        if os.path.exists(discarded_file):
            with open(discarded_file, 'r', encoding='utf-8') as f:
                discarded_data = json.load(f)
                existing_data["discarded_samples"] = discarded_data.get("samples", [])
                print(f"기존 버려진 샘플: {len(existing_data['discarded_samples'])}개")
        
        # 이미 처리된 샘플 인덱스 수집
        for sample in existing_data["all_samples"]:
            existing_data["processed_indices"].add(sample.get("sample_index", -1))
        
        print(f"이미 처리된 샘플 인덱스: {len(existing_data['processed_indices'])}개")
    ...
    def generate_sft_data(self, num_samples: int = 2):
        ...
        # 샘플 선택 (기존에 처리되지 않은 것들만)
        selected_samples = self.select_samples(reranking_data, num_samples, existing_data["processed_indices"])
        
        # 각 샘플 처리
        all_results = existing_data["all_samples"].copy()
        kept_samples = existing_data["kept_samples"].copy()
        discarded_samples = existing_data["discarded_samples"].copy()
        
        # 새로운 샘플 처리
        new_sample_start_idx = len(existing_data["all_samples"])
        
        for i, sample in enumerate(selected_samples):
            try:
                result = self.process_sample(sample, new_sample_start_idx + i)
                ...
                print(f"샘플 {new_sample_start_idx + i} 처리 중 오류: {e}")
                continue
        ...
        metadata = {
            ...
            "new_samples_added": len(selected_samples)
        }
        ...
        print(f"\n=== SFT 데이터 생성 완료 ===")
        print(f"전체 결과: {all_file} ({len(all_results)}개, 새로 추가: {len(selected_samples)}개)")
...
def main():
    ...
    generator = SFTDataGenerator(
        ...
        video_base_path="/home/work/smoretalk/seo/reranking/new_d/MSR-VTT/video",
        output_dir="./sft_data_contrastive_reasoning_unique_v1",
        openai_api_key=api_key
    )
    
    # SFT 데이터 생성 (기존 데이터에 이어서)
    sft_data = generator.generate_sft_data(num_samples=16)
    
    print("\n생성된 파일들:")
    print("- sft_data_contrastive_reasoning_unique_v1/sft_training_data.json")
    print("- sft_data_contrastive_reasoning_unique_v1/frames/ (프레임 이미지들)")
