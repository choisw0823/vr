...
def setup_model(
    ...
    if hasattr(config, "deepspeed") and config.deepspeed.enable and DEEPSPEED_AVAILABLE:
        ...
        # Use standard DDP or single GPU training when DeepSpeed is not available or disabled
        logger.info('Using standard distributed training without DeepSpeed')
        if config.distributed:
            from torch.nn.parallel import DistributedDataParallel as DDP
            model = DDP(model_without_ddp, device_ids=[config.gpu], find_unused_parameters=config.model.get("find_unused_parameters", False))
        else:
            model = model_without_ddp
        
        # Create optimizer and scheduler for non-DeepSpeed mode
        optimizer = create_optimizer(config.optimizer, model)
